            ...
            return new utils.Promise(function (resolve, reject) {
                var handleStateChange = function (nextState, previousState) {   
                    if (nextState.flowState !== previousState.flowState) {
                        ...
                        if (fromFlowState === previousState.flowState && toFlowState ===  currentState.flowState) {
                            resolve();
                        } else {
                            reject();
                        }
...
// with an object alongside with some convience functions  
...
function makeConnected () {
    ...
    var connected = {
        connect: function () {
            return new utils.Promise(function (resolve, reject) {
                ...
                options.onSessionCreated = function (session, sessionJoinType) {
                    log('Session created with a sessionJoinType of: ', sessionJoinType);
                    isJoiningSession = sessionJoinType === ConnectedLensSessionOptions.SessionCreationType.MultiplayerReceiver;
                };

                var getLocalUserIdOnce = utils.once(function (session, onComplete) {
                ...
                options.onError = function (session, code, message) {
                    if (message === "LensStudioConnectionError") {
                        log('Warning: LensStudio can\'t connect to connectedLens sessions itself. Pretending the session was connected but the session will be null.');
                        resolve();
                        ...
                        reject(message);
                        events.trigger(EventType.CONNECTED_LENS_ERROR, code, message);
                    }
                };

                // On message
                options.onMessageReceived = function (session, userId, message) {
        ...
        storeSessionType: function (sessionType) {
            ...
            if (multiplayerSession && typeof multiplayerSession.setStoredValue === 'function' ) {
                multiplayerSession.setStoredValue(
                    'connected-lens-session-type', 
                    sessionType,
                    storageOptions,
                    function() {
                        log('Successfully set session storage to session type:', sessionType);
                    }, 
                    function(error, description) {
                        log("Failed to set session storage to session type: " + error + ", " + description);
                    }
                );
            }
        },
        requestSessionType: function () {
            return new utils.Promise (function (resolve, reject) {
                multiplayerSession.getStoredValue(
                    ...
                    function(error, description) {
                        log("Error retrieving storage session type:", error, ",", description, "Will start remote");
...
//    var component = script.colocatedTrackingComponent; 
    ...
    updateEvent.bind(function () {
        ...
        if (hasBuildingProgress && latestBuildingProgress !== lastBuildingProgress) {
            lastBuildingProgress = latestBuildingProgress;
            events.trigger(EventType.BUILDING_PROGRESS_UPDATE, lastBuildingProgress);
        }
    });
        ...
        build: function (session) {
            return new utils.Promise(function (resolve, reject) {
                if (!component.canBuild) {
                    ...
                    return reject('Sorry, your device does not support shared AR experiences.');
                }
                            ...
                            colocated
                                ...
                                .then(function () {
                                    if (colocated.getIsTracking()) {
                                        ...
                                        actions.setFlowState(FlowState.JOINER_WAITING_FOR_TRACKING);
...
//                                        script.deviceTrackingComponent.enabled = false;
                                ...
                                .catch(function (message) {
                                    log('Failed to join a colocated connected lens session: ' + message);
                                    actions.setFlowState(FlowState.ERRORED);
                                    actions.setErrorMessage(typeof message !== 'undefined' ? 'Failed to join shared AR experience: ' + message : 'Failed to join shared AR experience.');
                                });
                        } else if (value === SessionType.REMOTE) {
//                            script.deviceTrackingComponent.enabled = true;
                            actions.setMultiplayerSession(connected.getMultiplayerSession());
                            actions.setSessionType(SessionType.REMOTE);
                            actions.setFlowState(FlowState.DONE)
                        } else {
                            actions.setFlowState(FlowState.ERRORED);
                            actions.setErrorMessage("Unknown session type of: " + value);
                        }
                    })
                    .catch(function (message) {
                        actions.setFlowState(FlowState.ERRORED);
...
script.api.startColocated = function () {
    ...
    actions.waitForFlowStateTransition( FlowState.PROMPT_TO_CREATE_COLOCATED, FlowState.CREATING_COLOCATED )
        .then(function () {

            if (waitingForConnectedSessionPromise) {
                log('User want to create a map, but are waiting for a session. Will keep waiting for the session.');
            ...
            function createMap () {
                ...
                function waitForBuildingComplete (progress) {   
                    if (progress >= 1) {
                        ...
                        colocated.offBuildingProgressUpdate(waitForBuildingComplete);
                        if (actions.getCurrentState().flowState === FlowState.CREATING_COLOCATED) {
                            log('Will set state to uploading colocated');
                            actions.setFlowState(FlowState.UPLOADING_COLOCATED);
                        } else {
                            log('Warning building is complete, but flowstate is not creating colocated');
                        }
                    }
                }
                    ...
                    .then(function () {
                        ...
                        if (actions.getCurrentState().snapCodeTexture === null) {
                            if (connected.getMultiplayerSession()) {
                                actions.setFlowState(FlowState.ERRORED);
                                actions.setErrorMessage("A snapcode has not been generated");
                            } else {
                                // Lens Studio Preview behaviour
                                log('I am running in Lens Studio Preview, I don\'t need a Snapcode');
                                actions.setFlowState(FlowState.COLOCATED_CREATED_SNAPCODE);

                            }
                        } else {
                            actions.setFlowState(FlowState.COLOCATED_CREATED_SNAPCODE);
                        }
                        actions.waitForFlowStateTransition(FlowState.COLOCATED_CREATED_SNAPCODE, FlowState.SNAPCODE_USED)
                            .then(function () {
                                if (colocated.getIsTracking()) {
                                    setDone();
                                } else {
                                    actions.setFlowState(FlowState.INITIATOR_WAITING_FOR_TRACKING);
                                    colocated.waitForTracking().then(setDone);
                                }
                                function setDone () {
                                    actions.setFlowState(FlowState.DONE);
//                                    script.deviceTrackingComponent.enabled = false;
        ...
        .catch(function (message) {
            var errorString = "Error at state: " + actions.getCurrentState().flowState + '. Error message: ' + message;
            log(errorString);
...
script.api.doConfirmStartBulding = function () {
    if (actions.getCurrentState().flowState === FlowState.PROMPT_TO_CREATE_COLOCATED) {
        actions.setFlowState(FlowState.CREATING_COLOCATED);
    } else {
        log('Something has asked to confirm building, but the UI is in the wrong state');
    }
}

// Confirm the user has finished with the snapcode
script.api.doConfirmSnapcodeUsed = function () {
    if (actions.getCurrentState().flowState === FlowState.COLOCATED_CREATED_SNAPCODE) {
        actions.setFlowState(FlowState.SNAPCODE_USED);
    } else if (actions.getCurrentState().flowState === FlowState.RESHOW_SNAPCODE) {
        actions.setFlowState(FlowState.DONE);
    } else {
        log('Something has asked to confirm the snapcode display is done with, but the UI is in the wrong state');
    }
}
...
//    script.deviceTrackingComponent.enabled = true;
    if (connected && typeof connected.getConnectedLensModule().shareSession === 'function') {
        actions.setSessionType(SessionType.REMOTE);
        connected.storeSessionType(SessionType.REMOTE);
        connected.getConnectedLensModule().shareSession(ConnectedLensModule.SessionShareType.Invitation, function () {});
